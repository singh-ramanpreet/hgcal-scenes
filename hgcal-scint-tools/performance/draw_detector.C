...
void draw_detector(const char* fname, int column, const char* label, const char* pref=0) {
  ...
  while (f && !feof(f)) {
    ...
    for (int binY=binYa;binY<=binYb;binY++) {
      double myval=cols[column];
      if (min<0 || myval<min) min=myval;
      if (max<myval) max=myval;
      if (column==6 && myval<1) myval=1;
